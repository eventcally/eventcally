...
class OrganizationVerificationRequestedEmailEventHandler(AbstractEventHandler):
    def __init__(self, organization_service: OrganizationApplicationService):
        super().__init__()
        self.organization_service = organization_service

    def handle(
        self, event: events.OrganizationVerificationRequested, uow: AbstractUnitOfWork
    ):
        self.organization_service.send_template_mails_to_members_async(
            uow,
            event.target_admin_unit_id,
