...
class EventReferenceRequestAutoVerifiedEmailEventHandler(AbstractEventHandler):
    def __init__(
        self,
    ...
    def handle(
        self, event: events.EventReferenceRequestAutoVerified, uow: AbstractUnitOfWork
    ):
        references = uow.event_references.get_by_event_id(event.event_id)
        reference = next(
            (r for r in references if r.admin_unit_id == event.admin_unit_id), None
        )

        if not reference:  # pragma: no cover
            return
