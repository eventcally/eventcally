...
def pytest_generate_tests(metafunc):
    ...
    # The production Dockerfile sets SECRET_KEY="" so it must be supplied at deploy
    # time; that empty-but-present value defeats project/__init__.py's own fallback
    # default (os.environ.get only substitutes when the key is absent, not empty),
    # which breaks any session/CSRF-dependent test when run from that image.
