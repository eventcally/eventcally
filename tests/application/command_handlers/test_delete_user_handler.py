...
from project.application.command_handlers.delete_user_handler import DeleteUserHandler
...
class TestDeleteUserHandler:
    def test_removes_user(self, uow):
        _make_actor_as_platform_admin(uow)
        _make_target_user(uow)
        cmd = commands.DeleteUserCommand.model_construct(actor=ACTOR, id=TARGET_USER_ID)

        DeleteUserHandler().handle(cmd, uow)

        assert uow.users.get(TARGET_USER_ID) is None

    def test_not_found_raises(self, uow):
        _make_actor_as_platform_admin(uow)
        cmd = commands.DeleteUserCommand.model_construct(actor=ACTOR, id=9999)
